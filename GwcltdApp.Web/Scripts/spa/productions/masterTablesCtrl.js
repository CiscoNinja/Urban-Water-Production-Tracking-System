(function (app) {
    ...
    app.controller('masterTablesCtrl', masterTablesCtrl);

    masterTablesCtrl.$inject = ['$scope', '$routeParams', 'apiService', 'notificationService', '$timeout', '$rootScope'];

    function masterTablesCtrl($scope, $routeParams, apiService, notificationService, $timeout, $rootScope) {
        $scope.loadingStatistics = false;
        $scope.wStations = [];
        $scope.msTable = { ID: $rootScope.repository.loggedUser.stationid }
        $scope.waterType = "";
        $scope.buttonVal = "";
        $scope.excelName = "";
        ...
        $(document).on('click', '.mybtn', function (event) {
            ...
            apiService.get('./api/productions/summary/' + $scope.msTable.ID + '/' + $scope.listOption.ID + '/' + $scope.buttonVal, null,
